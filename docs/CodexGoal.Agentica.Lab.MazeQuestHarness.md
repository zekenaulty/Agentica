# Codex Goal: Agentica.Lab MazeQuest Harness

> Lifecycle: **Implemented** · Completion: **95%** · Canonical status: [Agentica Product Status And Goal Xref](Agentica.ProductStatus.md)

## Mission

Build a host-provided MazeQuest test harness inside `Agentica.Lab`.
...
This is not an Agentica runtime feature. It is a Lab host scenario that supplies domain state, tools, rendering, and optional narration around Agentica's existing planner/executor loop.
...
The other Agentica implementation thread owns runtime changes. If the MazeQuest harness reveals missing runtime capabilities, document the need and work around it in the Lab where reasonable.
...
- Implement pause/stop in the Lab host loop, not in Agentica runtime.
...
```text
Agentica.Lab/Scenarios/MazeQuest/
```
...
```powershell
dotnet run --project Agentica.Lab -- mazequest list
dotnet run --project Agentica.Lab -- mazequest run --watch
dotnet run --project Agentica.Lab -- mazequest run sun_gate_maze --planner deterministic
dotnet run --project Agentica.Lab -- mazequest run sun_gate_maze --planner gemini --watch
```
...
- `mazequest run` may omit the quest id; the Lab command defaults to the first board quest and builds the host `RunRequest` from the generated quest objective.
- If `--planner` is omitted and console input is interactive, the Lab command asks `Run Gemini-backed MazeQuest planner test? [y/N]:` before the run starts.
...
- Agentica can already run a maze harness if the Lab host supplies tools and a deterministic planner.
...
These runtime-facing needs are now mostly satisfied for deterministic and first Gemini smoke runs. Remaining watch controls and narration are Lab host work, not runtime blockers.
...
- Added `Agentica.Lab/Scenarios/MazeQuest/`.
...
- Added no-quest-id self-start for `mazequest run`; the Lab command picks the first quest board entry and creates the RunRequest itself.
...
dotnet run --project Agentica.Lab -- mazequest list
dotnet run --project Agentica.Lab -- mazequest preview sun_gate_maze --type unlock --seed 173 --reveal
dotnet run --project Agentica.Lab -- mazequest run sun_gate_maze --planner deterministic --type unlock --seed 173 --planning-mode stepwise --timeout-seconds 120
dotnet run --project Agentica.Lab -- mazequest run sun_gate_maze --planner deterministic --type collect --seed 321 --planning-mode stepwise --timeout-seconds 120
dotnet run --project Agentica.Lab -- mazequest run --watch --narrator deterministic --turn-json --type unlock --seed 173 --width 7 --height 7 --planning-mode stepwise --timeout-seconds 60
dotnet run --project Agentica.Lab -- mazequest run --watch --type unlock --seed 173
```

If Gemini credentials are present:

```powershell
dotnet run --project Agentica.Lab -- run "Create a two-step workflow that queries state and then acts" --planner gemini --thinking-budget off
dotnet run --project Agentica.Lab -- mazequest run sun_gate_maze --planner gemini --type unlock --seed 173 --width 7 --height 7 --planning-mode plan-only --thinking-budget off --timeout-seconds 60
```
