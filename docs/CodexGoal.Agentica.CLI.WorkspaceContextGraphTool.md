# Codex Goal: Agentica.CLI Workspace Context Graph Tool
...
Add a read-only workspace discovery tool to the `Agentica.CLI` chat host.
...
Add this tool to the CLI chat host:
...
- `Agentica.CLI/Program.cs` routes `chat` to `ChatCommand.RunAsync`.
- `Agentica.CLI/Chat/ChatCommand.cs` resolves a `ChatSession`, stores the user message, builds a `RunRequest`, creates an `AgenticaRunner`, and passes `ChatTools.CreateCatalog(...)`.
...
WorkspaceContextGraph records under Agentica.CLI/Chat/
```
...
dotnet run --project Agentica.CLI -- chat "Discover the workspace context graph and summarize the manifest kinds you found." --planner gemini --workspace $root --new
```
...
1. `workspace.context.discover` is registered in the CLI chat tool catalog.
...
```powershell
dotnet run --project Agentica.CLI -- chat --help
dotnet run --project Agentica.CLI -- chat "hello" --planner deterministic --new
```

Run live Gemini workspace discovery only when credentials are intentionally configured:

```powershell
dotnet run --project Agentica.CLI -- chat "Use workspace.context.discover and summarize the discovered workspace context graph." --planner gemini --workspace C:\Users\Zythis\source\repos\Agentica --new
```
