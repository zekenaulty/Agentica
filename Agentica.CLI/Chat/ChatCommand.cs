...
internal static class ChatCommand
{
        ...
        CliCommandServices services)
    {
        ...
        var selectedPlanner = options.Planner ??
            (services.GeminiCredentialsAvailable() ? PlannerKind.Gemini : PlannerKind.Deterministic);
        ...
        ChatPersona persona)
    {
        ...
        return new Dictionary<string, object?>(StringComparer.Ordinal)
        {
            ...
            ["operatorContract"] =
                "Answer as a chat turn. Use tools for missing current facts. Use chat.context.append_note or chat.memory.summarize only when the user asks to preserve memory or a durable summary. Use workspace.image.create for image creation requests that need recent-chat/style/subject/layer composition; use workspace.image.generate only for already-shaped final image prompts. Include saved paths in the response. Do not claim a tool result unless a receipt or artifact exists. End by emitting chat.response.emit."
        };
    }
    ...
    private static void PrintUsage()
    {
        ...
        Console.Error.WriteLine("  Agentica.CLI chat --personas");
    }
    ...
    private static void PrintInteractiveHelp()
    {
        ...
        Console.WriteLine("image requests    Ask normally; artist-composed images are saved under workspace/images.");
