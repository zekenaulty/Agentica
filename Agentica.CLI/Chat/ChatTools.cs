        ...
        ToolCatalog.Create(
            ...
            Registration(
                ...
                new WorkspaceImageCreateTool(
                    ...
                    persona,
                    workspaceRoot,
                    new ChatArtistPromptComposer(new GeminiLlmClient()),
                    new GeminiImageGenerationClient()),
                "Compose a bounded artist brief from recent chat, save the final prompt, generate an image, and return a durable workspace image artifact.",
                ToolInputSchema.Create(
            ...
            Registration(
                ...
                ToolKind.Action,
                ToolEffect.WritesLocalState,
                new WorkspaceImageGenerateTool(store, conversation, workspaceRoot, new GeminiImageGenerationClient()),
                "Generate an image from an already-shaped prompt with Gemini native image generation, save it under the active workspace, and return a durable image artifact with paths.",
                ToolInputSchema.Create(
...
internal sealed class WorkspaceFileReadTool : ITool
{
    ...
    public async Task<ToolResult> ExecuteAsync(ToolInvocation invocation, CancellationToken cancellationToken)
    {
        ...
        if (!TryResolveWorkspacePath(_workspaceRoot, path, out var resolvedPath, out var error))
        {
        ...
        {
            return Refused(invocation, $"File does not exist under workspace: {path}");
        }
...
internal sealed class WorkspaceFileSearchTool : ITool
{
    ...
    public async Task<ToolResult> ExecuteAsync(ToolInvocation invocation, CancellationToken cancellationToken)
    {
        ...
        var path = ChatToolInput.String(invocation.Input, "path");
        if (!TryResolveWorkspacePath(_workspaceRoot, path, out var searchRoot, out var error))
        {
            return Refused(invocation, error);
        }

        var maxResults = ChatToolInput.Int(invocation.Input, "maxResults", 40, 1, 200);
        var (matches, usedFallback) = await SearchAsync(searchRoot, pattern, maxResults, cancellationToken).ConfigureAwait(false);
        var data = new Dictionary<string, object?>
        {
            ["pattern"] = pattern,
            ["path"] = searchRoot,
    ...
    private static async Task<(IReadOnlyList<string> Matches, bool UsedFallback)> SearchAsync(
        string searchRoot,
        string pattern,
        int maxResults,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var lines = output
                ...
                .Take(maxResults)
                .ToArray();
            return (lines, UsedFallback: false);
        }
        catch
        {
            return (FallbackSearch(searchRoot, pattern, maxResults), UsedFallback: true);
        }
    }

    private static IReadOnlyList<string> FallbackSearch(string searchRoot, string pattern, int maxResults)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(searchRoot, "*", SearchOption.AllDirectories))
        {
            ...
            for (var index = 0; index < lines.Length && matches.Count < maxResults; index++)
            {
                ...
                {
                    matches.Add($"{file}:{index + 1}:1:{lines[index]}");
                }
            }
        }
...
internal sealed class WorkspaceImageCreateTool : ITool
{
    ...
    public async Task<ToolResult> ExecuteAsync(ToolInvocation invocation, CancellationToken cancellationToken)
    {
        ...
        var promptPlanPath = await SavePromptPlanAsync(promptPlanData, cancellationToken).ConfigureAwait(false);
    ...
    private async Task<string> SavePromptPlanAsync(
        ...
        CancellationToken cancellationToken)
    {
        var promptDirectory = Path.Combine(_workspaceRoot, "images", "prompts");
        Directory.CreateDirectory(promptDirectory);
        var createdAt = DateTimeOffset.UtcNow;
        var baseName = $"{createdAt:yyyyMMdd_HHmmss}_{Guid.NewGuid():N}"[..24];
        var path = Path.Combine(promptDirectory, $"{baseName}.artist.json");
    ...
    public async Task<ToolResult> ExecuteAsync(ToolInvocation invocation, CancellationToken cancellationToken)
    {
        ...
        try
        {
            saved = await ChatImageToolSupport.GenerateAndSaveAsync(
                    ...
                    ChatToolIds.WorkspaceImageGenerate,
                    additionalData: null,
                    additionalRequestMetadata: null,
                    cancellationToken)
                .ConfigureAwait(false);
        }
        catch (LlmClientException exception)
...
internal static class ChatToolHelpers
{
        ...
        out string error)
    {
        ...
        var candidate = resolvedPath.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar) +
            (Directory.Exists(resolvedPath) ? Path.DirectorySeparatorChar : string.Empty);
