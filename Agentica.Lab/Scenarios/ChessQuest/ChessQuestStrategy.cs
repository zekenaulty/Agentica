...
public sealed class ChessQuestPhaseCompletionEvaluator : ICompletionEvaluator
{
    ...
    public CompletionEvaluation Evaluate(CompletionContext context)
    {
        if (context.Artifacts.FirstOrDefault(artifact =>
                string.Equals(artifact.Kind, "chessquest.objective_completed", StringComparison.Ordinal)) is { } artifact)
        {
            return CompletionEvaluation.Complete(new EvidenceRef("artifact", artifact.ArtifactId));
        }
