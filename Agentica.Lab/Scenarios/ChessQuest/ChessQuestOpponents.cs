...
public sealed class PlannerChessOpponent : IChessOpponent
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var runner = new AgenticaRunner(
            ...
            ChessQuestFirstMoveCompletionEvaluator.Instance,
    ...
    private static string BuildObjective(ChessOpponentRequest request) =>
        ...
        Choose exactly one legal coordinate-UCI move for {request.OpponentColor} from the current public board state.
...
internal sealed class ChessQuestFirstMoveCompletionEvaluator : ICompletionEvaluator
{
    ...
    {
        var receipt = context.Receipts.FirstOrDefault(candidate =>
            string.Equals(candidate.ToolId, ChessQuestToolIds.PlayMove, StringComparison.Ordinal) &&
            candidate.Status == ReceiptStatus.Succeeded);
