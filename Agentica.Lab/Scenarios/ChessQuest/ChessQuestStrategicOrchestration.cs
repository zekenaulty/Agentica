...
public sealed class ChessQuestPhaseRunExecutor : IRunExecutor
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var planner = _plannerFactory(phaseRequest);
        var runner = new AgenticaRunner(
            ...
            _policy with { SecurityPolicy = LabSecurityPolicy.ForPlanner(planner) },
            new ChessQuestPhaseCompletionEvaluator(_state.Session, phaseTracker),
...
public sealed class ChessQuestTaskAcceptanceEvaluator : ITaskAcceptanceEvaluator
{
    ...
    private static IReadOnlyList<EvidenceRef> Evidence(OutcomeEnvelope outcome) =>
        outcome.Details.Artifacts
            .Where(artifact => string.Equals(artifact.Kind, "chessquest.phase_report", StringComparison.Ordinal))
            .Select(artifact => new EvidenceRef("artifact", artifact.ArtifactId))
            .Concat(outcome.Receipts.Items
