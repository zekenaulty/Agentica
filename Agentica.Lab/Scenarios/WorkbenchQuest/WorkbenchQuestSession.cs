...
public sealed record WorkbenchCurrentStateCheck(
    bool Passed,
    string Output);
...
public sealed class WorkbenchQuestSession
{
    ...
    public WorkbenchCurrentStateCheck EvaluateCurrentState()
    {
        try
        {
            ...
            return new WorkbenchCurrentStateCheck(passed, output);
        }
        catch (Exception exception) when (exception is not OutOfMemoryException)
        {
            return new WorkbenchCurrentStateCheck(
                Passed: false,
                Output: $"FAIL authoritative checker error: {exception.GetType().Name}");
        }
    }
    ...
    {
        var currentStateCheck = EvaluateCurrentState();
        var passed = currentStateCheck.Passed;
        var output = currentStateCheck.Output;
    ...
    {
        var currentStateCheck = EvaluateCurrentState();
        var blockers = CompletionBlockers(currentStateCheck);
        if (blockers.Count > 0)
        {
            return Refused(
                invocation,
                ...
                new Dictionary<string, object?>(StringComparer.Ordinal)
                {
                    ...
                    ["currentCheckStatus"] = currentStateCheck.Passed ? "passed" : "failed",
                    ["currentCheckOutput"] = currentStateCheck.Output
                });
        }
        ...
        data["completionEvidence"] = new Dictionary<string, object?>(StringComparer.Ordinal)
        {
            ...
            ["passingLatestCheckAfterLastPatch"] = true,
            ["currentStateCheckPassed"] = true
        };
    ...
    private IReadOnlyList<string> CompletionBlockers(WorkbenchCurrentStateCheck currentStateCheck)
    {
        ...
        if (lastPatch is not null &&
            (latestCheck is null || !latestCheck.Passed || latestCheck.ActionOrder <= lastPatch.ActionOrder))
        {
            blockers.Add("The latest check is not a passing check recorded after the last applied patch.");
        }

        if (!currentStateCheck.Passed)
        {
            blockers.Add("The current workbench state does not pass its authoritative checker.");
        }
